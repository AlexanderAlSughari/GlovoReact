...
const Navbar = () => {
  ...
  return (
    <div className='flex justify-between items-center py-5 px-0'>
      <img src={assets.logo} alt="logo" className="w-[150px] laptop:w-[140px] tablet:w-[120px]" />
      ...
      <div className="flex items-center gap-10 laptop:gap-[30px] tablet:gap-[20px]">
        ...
        <div className="relative">
          <img className="laptop:w-[22px] tablet:w-[20px]" src={assets.basket_icon} alt="basket_icon" />
          <div className="absolute min-w-[10px] min-h-[10px] bg-[#9ACD32] rounded-[5px] -top-2 -right-2"></div>
        </div>
        <button className='bg-transparent text-base text-[#49557e] border border-solid border-[#9ACD32] py-[10px] px-[30px] rounded-[50px] cursor-pointer hover:bg-[#f2fff4] hover:duration-300 laptop:p-[8px_25px] tablet:p-[7px_20px] tablet:text-[15px]'>Zaloguj się</button>
      </div>
    </div>
  )
}
