...
const Navbar = ({setShowLogin}) => {
  ...
  return (
    <div className='flex justify-between items-center py-5 px-0 dark:bg-black dark:text-color-white' id='navbar'>
      ...
      <ul className='flex list-none gap-5 text-[#49557e] text-lg laptop:gap-5 laptop:text-[17px] tablet:gap-[15px] tablet:text-base phone:hidden'>
        <Link to='/'             onClick={() => setMenu("home")}    className={`${menu === "home"    ? "pb-0.5 border-b-2 border-solid border-[#49557e] dark:text-[#939BC3]" : "dark:text-[#939BC3]"} cursor-pointer`}>Strona główna</Link>
        <a href='/#explore-menu' onClick={() => setMenu("menu")}    className={`${menu === "menu"    ? "pb-0.5 border-b-2 border-solid border-[#49557e] dark:text-[#939BC3]" : "dark:text-[#939BC3]"} cursor-pointer`}>Menu</a>
        <a href='/#app-download' onClick={() => setMenu("app")}     className={`${menu === "app"     ? "pb-0.5 border-b-2 border-solid border-[#49557e] dark:text-[#939BC3]" : "dark:text-[#939BC3]"} cursor-pointer`}>Aplikacja</a>
        <a href='/#footer'       onClick={() => setMenu("contact")} className={`${menu === "contact" ? "pb-0.5 border-b-2 border-solid border-[#49557e] dark:text-[#939BC3]" : "dark:text-[#939BC3]"} cursor-pointer`}>Kontakt</a>
      </ul>
      <div className="flex items-center gap-10 laptop:gap-[30px] tablet:gap-[20px]">
      <button onClick={()=> darkModeHandler()}>
      {
          
          dark && <IoSunny className='fill-current text-white'/> 
      }
      {
          !dark && <IoMoon /> 
      }
